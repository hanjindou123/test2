class UsersController < ApplicationController
  before_action :set_user, only: [:show, :edit, :update, :destroy]

  # GET /users
  # GET /users.json
  def index
    @users = User.all
  end

  # GET /users/1
  # GET /users/1.json
  def show
  end

  # GET /users/new
  def new
    @user = User.new
  end

  # GET /users/1/edit
  def edit
  end

  # POST /users
  # POST /users.json
  def create
  username = params[:user][:name]
    @userer = User.find_by_user_loginname(username)
    if @userer.class == NilClass
    @user = User.new(user_params)
    
    respond_to do |format|
      if @user.save
        format.html { redirect_to @user, notice: 'User was successfully created.' }
        format.json { render :show, status: :created, location: @user }
        session[:user_new_warning]=""
      else
        format.html { render :new }
        format.json { render json: @user.errors, status: :unprocessable_entity }
      end
    end
    else
        session[:user_new_warning]="There is a user has a same username with you,please change your username!"
        redirect_to :action => "new"
        @notice = "There is a user has a same username with you,please change your username!"
    end
    
  end

  # PATCH/PUT /users/1
  # PATCH/PUT /users/1.json
  def update
    respond_to do |format|
      if @user.update(user_params)
        format.html { redirect_to @user, notice: 'User was successfully updated.' }
        format.json { render :show, status: :ok, location: @user }
      else
        format.html { render :edit }
        format.json { render json: @user.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /users/1
  # DELETE /users/1.json
  def destroy
    @user.destroy
    respond_to do |format|
      format.html { redirect_to users_url, notice: 'User was successfully destroyed.' }
      format.json { head :no_content }
    end
  end

def login
    unless request.get?
        username = params[:user][:user_loginname]
        password = params[:user][:user_password]
        session[:user_name]=username
        @hint = "ok"
        @user = User.find_by_user_loginname(username)
        if @user.class == NilClass
          @hint = 'Unregisted Username!'
        elsif @user.user_password == params[:user][:user_password]
          @hint = "Welcome back, "
          if session[:backurl]==nil
            redirect_to instruments_path
          else
            redirect_to session[:backurl]
            session[:backurl]=nil
          end
        else
          @hint = "Login failed, please check up you info"
        end        
    end
  end

  def logout
    
      session[:user_name]=nil
      redirect_to instruments_path

  end

  private
    # Use callbacks to share common setup or constraints between actions.
    def set_user
      @user = User.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def user_params
      params.require(:user).permit(:user_loginname, :user_password, :user_relatecount, :user_email, :user_realname, :user_address, :user_phonenumber, :user_payable)
    end
end
